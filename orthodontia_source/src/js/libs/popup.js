...
class Popup {
	...
	eventsPopup() {
		...
		document.addEventListener("click", function (e) {
			...
			if (buttonOpen) {
				e.preventDefault();
        let dataValueTemp = buttonOpen.getAttribute(this.options.attributeOpenButton) ?
        ...
        dataValueTemp = dataValueTemp.indexOf('#')<0&&dataValueTemp.indexOf('.')<0 ? `#${dataValueTemp }` : dataValueTemp;
          ...
          this.targetOpen.trigger = buttonOpen;
          this.open();
					return;
      ...
      if (this.targetOpen.element&&!this.targetOpen.element.hasAttribute('data-noclose')) {
        if (buttonClose || !e.target.closest(`.${this.options.classes.popupContent}`) && this.isOpen) {
          if (!e.target.closest('.tooltip-calendar')&&!e.target.closest('[data-mobile-dropdown]')) {
            e.preventDefault();
            this.close();
            return;
          }
	...
	open(selectorValue) {
		if (bodyLockStatus) {
			...
			if (this.targetOpen.element) {
				...
				if (this.options.hashSettings.location) {
					...
					this._getHash();
          if (this.targetOpen.element&&!this.targetOpen.element.hasAttribute('data-nohash')) {
            this._setHash();
          }
				}
        ...
        if (this.targetOpen.element.id === 'calendarPopup') {
  ...
  _openToHash() {
    ...
    const elementId = document.getElementById(hash);
    const classInHash = elementId ? `#${hash}` : `.${hash}`;
  
    const element = document.querySelector(classInHash);
    const buttons = element ? document.querySelector(`[${this.options.attributeOpenButton}="${classInHash}"]`) || document.querySelector(`[${this.options.attributeOpenButton}="${classInHash.replace('.', '#')}"]`) : null;
    const condition = element && !element.hasAttribute('data-nohash');
  
    if (buttons && element && condition) {
